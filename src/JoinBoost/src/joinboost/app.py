...
class DummyModel(App):
    ...
    def fit(self, jg: JoinGraph, skip_preprocess=False, warm_start=False, filter_expression=None):
        self._fit(jg, skip_preprocess=skip_preprocess, warm_start=warm_start, filter_expression=filter_expression)
    ...
    def _fit(self, jg: JoinGraph, skip_preprocess=False, warm_start=False, filter_expression=None):
        ...
        if filter_expression:
            jg.add_annotation(jg.target_relation, filter_expression)
        ...
        if warm_start and hasattr(self, "constant_"):
            pred_agg = self.get_prediction_aggregate()
            residual_agg = AggExpression(Aggregator.SUB, (jg.target_var, pred_agg))
            agg_exp = self.semi_ring.col_sum((residual_agg, "1"))
            spja_data = SPJAData(
                aggregate_expressions=agg_exp, from_tables=[jg.target_relation], 
                qualified=False, select_conds=select_conds
            )
        else:
        ...
        self.count_ = h
        if not warm_start or not hasattr(self, "constant_"):
...
class DecisionTree(DummyModel):
    ...
    def _fit(self, jg: JoinGraph, skip_preprocess=False, warm_start=False, filter_expression=None):
        ...
        # substracting the target variable by residuals
        if warm_start:
            pred_agg = self.get_prediction_aggregate()
        ...
        self.cjt.lift(agg_to_sql(exp, qualified=False))
        if not warm_start:
...
class GradientBoosting(DecisionTree):
    ...
    def _update_error(self):
        ...
        for cur_cjt in self.leaf_nodes:
            ...
            all_conds = join_conds + cur_cjt.get_annotations(target_relation)
            case_conditions.append((pred, all_conds))
        
        if case_conditions:
            # Create a CASE expression for the update
            update_exp = f"{g_col} = {g_col} - (" + agg_to_sql(AggExpression(Aggregator.CASE, case_conditions), qualified=False) + ")"
            # We don't need a WHERE clause for the whole update because the CASE handles it with ELSE 0
            self.cjt.exe._execute_query(f"UPDATE {target_relation} SET {update_exp}")
        
        # update the semi-ring to reflect the new total gradient
        g, h = self.semi_ring.get_value()
        self.semi_ring.set_semi_ring(g * (1 - self.learning_rate), h)
